...
from database import log_backup, log_sync, get_database_size, DATABASE_PATH, get_dropbox_tokens, update_dropbox_access_token
import dropbox_auth

load_dotenv()

DROPBOX_BACKUP_FOLDER = "/kinolog_backups"
...
async def backup_to_dropbox() -> dict:
    ...
    try:
        ...
        backup_filename = f"kinolog_backup_{timestamp}.zip"
        ...
        await log_sync("backup", "success", f"Создан бэкап: {backup_filename}")
        ...
        return {
            ...
            "message": f"Бэкап создан: {backup_filename}"
        }

    except Exception as e:
        await log_backup("error", error_message=str(e))
        await log_sync("backup", "error", str(e))
...
async def get_backup_status() -> dict:
    from database import get_db, get_all_backups, get_sync_status

    backups = await get_all_backups()
...
async def list_dropbox_backups() -> list:
    dbx = await _get_dropbox_client()
    if not dbx:
        return []
        ...
        for file in files:
            if isinstance(file, dropbox.FileMetadata):
                backups.append({
                    "name": file.name,
                    "size": file.size,
                    "modified": file.server_modified.isoformat(),
