...
async def log_sync(sync_type: str, status: str, message: str = None,
                   local_hash: str = None, remote_hash: str = None):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            """INSERT INTO sync_log (sync_type, status, message, local_hash, remote_hash)
               VALUES (?, ?, ?, ?, ?)""",
            (sync_type, status, message, local_hash, remote_hash)
        )
        await db.commit()
...
async def get_sync_status() -> dict:
    async with aiosqlite.connect(DATABASE_PATH) as db:
        ...
        if row:
            return dict(row)
        return {"sync_time": None, "sync_type": None, "status": "never", "message": "Синхронизация ещё не выполнялась"}


async def get_all_backups() -> list:
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
            "SELECT * FROM backup_log ORDER BY backup_time DESC LIMIT 20"
        )
        rows = await cursor.fetchall()
        return [dict(row) for row in rows]
